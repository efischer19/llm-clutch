...
Set up the llm-clutch library at the repository root following the
single-library structure defined in ADR-016. Initialize the project with uv
(per ADR-015), configure Ruff linting (ADR-005), and establish
the package directory layout that all subsequent tickets will build upon.

## Acceptance Criteria

- [ ] `pyproject.toml` exists with valid PEP 621 metadata (name: `llm-clutch`, Python >=3.12)
- [ ] Package source directory exists at `src/llm_clutch/` with an `__init__.py` exposing the package version
- [ ] Sub-package directories exist: `backend/`, `core/`, `integrations/` (each with `__init__.py`)
- [ ] `tests/` directory exists with a passing placeholder test
...
- [ ] A `README.md` exists with a brief project description

## Implementation Notes

- Follow the single-library structure from ADR-016 — the library lives at the
  repository root, not in `libs/`, since this is now a single-library project.
